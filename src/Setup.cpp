...
void Setup::encodeProperties(std::vector<Property>& properties)
{
	...
	properties.push_back(Property("setup.arduinoSerialPath", arduinoSerialPath));
	properties.push_back(Property("setup.arduinoBaudRate", ToString(arduinoBaudRate)));
	properties.push_back(Property("setup.feedRate", ToString(feedRate)));
...
void Setup::decodeProperties(const std::vector<Property>& properties)
{
	for (size_t iProp = 0; iProp < properties.size(); iProp++)
	{
		...
		else if (prop.name == "setup.arduinoSerialPath")
		{
			arduinoSerialPath = prop.value;
		}
		else if (prop.name == "setup.arduinoBaudRate")
		{
			arduinoBaudRate = ToInt(prop.value);
		}
		else if (prop.name == "setup.feedRate")
		{
			feedRate = ToInt(prop.value);
		}
