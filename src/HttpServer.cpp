...
// libmicrohttpd >= 0.9.71 switched callback return types from `int` to
// the strongly-typed `enum MHD_Result`.  Keep one macro so the same
// source compiles on both old Raspbian and modern Bookworm/Bullseye.
...
static MHD_RESULT BuildError(struct MHD_Connection *connection, const std::string& message, int httpCode = MHD_HTTP_INTERNAL_SERVER_ERROR)
{
...
static MHD_RESULT StorePostToMap (void *cls, enum MHD_ValueKind kind,
...
static MHD_RESULT StoreToMap (void *cls, enum MHD_ValueKind kind,
...
static void SaveSetup(RequestInfo * reqInfo)
{
	...
	std::string arduinoSerialPath = reqInfo->arguments[WebContent::ARDUINO_SERIAL_PATH];
	setup->arduinoSerialPath = arduinoSerialPath;

	std::string arduinoBaudRate = reqInfo->arguments[WebContent::ARDUINO_BAUD_RATE];
	if (!arduinoBaudRate.empty())
	{
		int baud = ToInt(arduinoBaudRate);
		if (baud <= 0)
		{
			throw Exception("Invalid Arduino baud rate.");
		}
		setup->arduinoBaudRate = baud;
	}

	std::string feedRate = reqInfo->arguments[WebContent::FEED_RATE];
	if (!feedRate.empty())
	{
		int fr = ToInt(feedRate);
		if (fr <= 0)
		{
			throw Exception("Invalid Feed Rate.");
		}
		setup->feedRate = fr;
	}
...
static MHD_RESULT StartRequest(RequestInfo * reqInfo)
{
...
static MHD_RESULT ShowScanProgress(RequestInfo * reqInfo)
{
	...
	MHD_RESULT ret = MHD_queue_response (reqInfo->connection, MHD_HTTP_OK, response);
	MHD_destroy_response (response);
...
static MHD_RESULT RetrieveFile(RequestInfo * reqInfo, const std::string& url)
{
	std::string filename;
...
static MHD_RESULT LivePly(RequestInfo * reqInfo)
{
	HttpServer * server = reqInfo->server;
	Scanner * scanner = server->getScanner();
	MHD_RESULT ret = MHD_YES;
...
static MHD_RESULT GetRenderImage(RequestInfo * reqInfo)
{
...
static MHD_RESULT GetCameraImage(RequestInfo * reqInfo, bool addLines)
{
...
static MHD_RESULT ProcessPageRequest(RequestInfo * reqInfo)
{
	HttpServer * server = reqInfo->server;
	MHD_RESULT ret = MHD_YES;
	...
	if (!server->getScanner()->isRunning())
	{
		...
		else if (reqInfo->url.find("/setup") == 0)
		{
			...
			else if (reqInfo->method == RequestInfo::POST && cmd == "testArduino")
			{
				GCodeClient * client = GCodeClient::get();
				if (client == NULL || !client->isOpen())
				{
					message = "Arduino transport not connected. Save settings and try again.";
				}
				else if (client->ping(2000))
				{
					message = "Arduino OK (" + client->getDevicePath() + ").";
				}
				else
				{
					message = "Arduino did not respond on " + client->getDevicePath() + ".";
				}
			}
		...
		else if (reqInfo->url == "/" || reqInfo->url == "/preview") // We check preview here in case the scan just ended
		{
			...
			if (reqInfo->method == RequestInfo::POST && cmd == "startScan")
			{
				...
				server->startScan(ToReal(degrees.c_str()));
				Thread::usleep(500000);
			}
			else
			{
...
static MHD_RESULT ContinueRequest(RequestInfo * reqInfo)
{
	MHD_RESULT ret = MHD_YES;
...
static MHD_RESULT ConnectionHandler (void *cls, struct MHD_Connection *connection,
                                const char *url,
...
{
	// Tear down hardware ports BEFORE the transport so their destructors
	// can still send "off" commands; then rebuild the transport from the
	// current Setup before reconstructing the ports.
	Lighting::release();
	Laser::release();
	TurnTable::release();
	GCodeClient::release();
	...
	Setup * setup = Setup::get();
	if (!GCodeClient::initialize(setup->arduinoSerialPath, setup->arduinoBaudRate))
	{
		ErrorLog << "!! Arduino transport unavailable after reinitialize()." << Logger::ENDL;
	}
...
void HttpServer::startScan(real degrees)
{
	if (m_scanner->isRunning())
	{
		InfoLog << "Scan already in progress; ignoring start request." << Logger::ENDL;
		return;
	}

	m_scanner->setRange(degrees);
	m_scanner->setTask(Scanner::GENERATE_SCAN);
	m_scanner->execute();
