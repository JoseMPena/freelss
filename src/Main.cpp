...
struct InitSingletons
{
	...
	~InitSingletons()
	{
		...
		// Hardware ports must be released before the GCodeClient they
		// share so their destructors can still send "off" commands.
		freelss::Laser::release();
		freelss::Camera::release();
		freelss::TurnTable::release();
		freelss::Lighting::release();
		freelss::StatusGpio::release();
		freelss::GCodeClient::release();
...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		// Open the Arduino serial transport before any hardware port is
		// constructed (Laser / TurnTable / Lighting all share it).
		freelss::Setup * setup = freelss::Setup::get();
		if (!freelss::GCodeClient::initialize(setup->arduinoSerialPath, setup->arduinoBaudRate))
		{
			freelss::ErrorLog << "!! Arduino transport unavailable; hardware control disabled until reconnected." << freelss::Logger::ENDL;
		}
		...
		freelss::StatusGpio::get()->execute();
