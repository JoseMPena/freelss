...
const std::string WebContent::ARDUINO_SERIAL_PATH = "ARDUINO_SERIAL_PATH";
const std::string WebContent::ARDUINO_BAUD_RATE  = "ARDUINO_BAUD_RATE";
const std::string WebContent::FEED_RATE          = "FEED_RATE";
...
const std::string WebContent::STEP_DELAY_DESCR = "Legacy; only used to derive Feed Rate when Feed Rate is unset.";
const std::string WebContent::ARDUINO_SERIAL_PATH_DESCR = "USB device path for the Arduino Mega (e.g. /dev/ttyACM0). Leave empty to autodetect.";
const std::string WebContent::ARDUINO_BAUD_RATE_DESCR = "Baud rate of the Arduino G-code firmware (default 115200).";
const std::string WebContent::FEED_RATE_DESCR = "G-code feed rate (steps/second) passed to the Arduino on every G2 move.";
...
<div id=\"menuContainer\">\
  ...
  <div class=\"menu\"><a href=\"/settings\">SETTINGS</a></div>"
	...
	sstr << setting(WebContent::ARDUINO_SERIAL_PATH, "Arduino Serial Path", setup->arduinoSerialPath, ARDUINO_SERIAL_PATH_DESCR);
	sstr << setting(WebContent::ARDUINO_BAUD_RATE,   "Arduino Baud Rate",   setup->arduinoBaudRate,   ARDUINO_BAUD_RATE_DESCR);
	sstr << setting(WebContent::FEED_RATE,           "Feed Rate",            setup->feedRate,          FEED_RATE_DESCR, "steps/s");

	sstr << setting(WebContent::STEPS_PER_REVOLUTION, "Steps Per Revolution", setup->stepsPerRevolution, STEPS_PER_REVOLUTION_DESCR);
	sstr << setting(WebContent::DIRECTION_VALUE,      "Invert Direction",     setup->motorDirPinValue,   "Set to 1 to reverse turntable rotation direction.");
...
sstr << "<div style=\"margin-top: 1em;\"><form method=\"POST\" action=\"/setup\" style=\"display: inline\">"
<< "<input type=\"hidden\" name=\"cmd\" value=\"testArduino\">"
<< "<button type=\"submit\">Test Arduino Connection</button>"
<< "</form></div>\n";
